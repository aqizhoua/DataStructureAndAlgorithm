...
void ListManage::MergeList(const SequenceList &list2) {
    ...
    while(j<list_.length&&k<list2.length){
        new_array[i++]=list_.data[j].id<list2.data[k].id?list_.data[j++]:list2.data[k++];
    }
...
int main(){
    ...
    for (int i = 0; i < 5; ++i){
        student_info.id=i;
        list_manage.PushBack(student_info);
    }
    list_manage.PrintList();
    list_manage.PopFront();
    list_manage.PrintList();
    student_info.id=-1;
    list_manage.PushFront(student_info);
    list_manage.PrintList();
    list_manage.PopBack();
    student_info.id=1;
    list_manage.Insert(2,student_info);
    list_manage.PrintList();

    SequenceList sequence_list2;
    sequence_list2.data = new ElemType[2];
    sequence_list2.data[0].id=0;
    sequence_list2.data[1].id=4;
